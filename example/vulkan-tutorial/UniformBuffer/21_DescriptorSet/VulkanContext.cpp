...
VulkanContext::VulkanContext(const VulkanFeatureManager &feature, WindowFunc getWindows) :
    m_featureManager(feature), m_getWindowFunc(std::move(getWindows))
{
    init();
}
...
void VulkanContext::cleanup()
{
    ...
    {
        if (m_imageAvailableSemaphores.at(index) != VK_NULL_HANDLE)
        {
            vkDestroySemaphore(m_logicDevice, m_imageAvailableSemaphores.at(index), nullptr);
        }

        if (m_renderFinishedSemaphores.at(index) != VK_NULL_HANDLE)
        {
            vkDestroySemaphore(m_logicDevice, m_renderFinishedSemaphores.at(index), nullptr);
        }

        if (m_inFlightFences.at(index) != VK_NULL_HANDLE)
        {
            vkDestroyFence(m_logicDevice, m_inFlightFences.at(index), nullptr);
        }
    }
...
void VulkanContext::OnResize(uint32_t width, uint32_t height) { framebufferResized = true; }
...
void VulkanContext::pickPhysicalDevice()
{
    ...
    auto isDeviceSuitable = [&findQueueFamilies, querySwapChainSupportDetails, this](const VkPhysicalDevice &device)
    {
        ...
        const auto extensionSupported = m_featureManager.validateDeviceSupport(device);
        ; ///< 是否支持请求的那些扩展
...
void VulkanContext::createImageViews()
{
    ...
    {
        VkImageSubresourceRange sourceRange{.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
                                            .baseMipLevel = 0,
                                            .levelCount = 1,
                                            .baseArrayLayer = 0,
                                            .layerCount = 1};
...
void VulkanContext::createGraphicsPipeline()
{
    ...
    vertexInputStageCreatInfo.vertexBindingDescriptionCount = 1;
    vertexInputStageCreatInfo.vertexAttributeDescriptionCount = attributeDescriptions.size();
    vertexInputStageCreatInfo.pVertexBindingDescriptions = &bindingDescription;
    vertexInputStageCreatInfo.pVertexAttributeDescriptions = attributeDescriptions.data();
    ...
    VkPipelineRasterizationStateCreateInfo rasterizationStateCreateInfo{
            ...
            .frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE, /// 正面顶点逆时针
...
{
    VkAttachmentDescription colorAttachmentDescription{.flags = 0,
                                                       .format = m_swapChainImageFormat,
    ...
    VkSubpassDependency waitDependency{.srcSubpass = VK_SUBPASS_EXTERNAL,
                                       .dstSubpass = 0,
    ...
    renderPassInfo.pDependencies = &waitDependency;
...
{
    const size_t bufferSize = m_swapChainImageViews.size();
...
void VulkanContext::createCommandBuffers()
{
    ...
    if (vkAllocateCommandBuffers(m_logicDevice, &commandBufferAllocateInfo, m_commandBuffers.data()) != VK_SUCCESS)
    {
...
void VulkanContext::createSyncObjects()
{
    ...
    VkFenceCreateInfo fenceCreateInfo{
            ...
            .pNext = nullptr,
            .flags = VK_FENCE_CREATE_SIGNALED_BIT // 创建的是一个发出信号的栅栏 所以第一次等待的时候可以直接通过
    };
    ...
    for (auto index = 0; index < MAX_FRAMES_IN_FLIGHT; ++index)
    {
        if (vkCreateSemaphore(m_logicDevice, &semaphoreCreateInfo, nullptr,
                              m_imageAvailableSemaphores.data() + index) != VK_SUCCESS ||
            vkCreateSemaphore(m_logicDevice, &semaphoreCreateInfo, nullptr,
                              m_renderFinishedSemaphores.data() + index) != VK_SUCCESS ||
            vkCreateFence(m_logicDevice, &fenceCreateInfo, nullptr, m_inFlightFences.data() + index) != VK_SUCCESS)
        {
...
void VulkanContext::createVertexBuffers()
{
    ...
    VulkanUtils::createBuffer(m_physicalDevice, m_logicDevice, bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                              VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer,
                              stagingBufferMemory);


    /// map data to buffer -> by memcpy
    void *data = nullptr;
    vkMapMemory(m_logicDevice, stagingBufferMemory, 0, bufferSize, 0, &data);
    memcpy(data, m_vertices.data(), bufferSize);
    vkUnmapMemory(m_logicDevice, stagingBufferMemory);

    VulkanUtils::createBuffer(m_physicalDevice, m_logicDevice, bufferSize,
    ...
    auto copyBuffer = [this](const VkBuffer &srcBuffer, const VkBuffer &dstBuffer, const VkDeviceSize bufferSize)
    {
        ...
        if (vkAllocateCommandBuffers(m_logicDevice, &allocateInfo, &commandBuffer) != VK_SUCCESS)
        {
            throw std::runtime_error("Failed to allocate command buffers!");
        }
        ...
        /// copy image
        const VkBufferCopy copyRegion = {.srcOffset = 0, .dstOffset = 0, .size = bufferSize};
...
void VulkanContext::createUniformBuffers()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
        ...
        vkMapMemory(m_logicDevice, m_uniformBuffersMemory[i], 0, bufferSize, 0, &m_uniformBuffersMapped[i]);
    }
}
...
void VulkanContext::createDescriptorSet()
{
    ...
    if (vkAllocateDescriptorSets(m_logicDevice, &allocateInfo, m_descriptorsets.data()) != VK_SUCCESS)
    {
