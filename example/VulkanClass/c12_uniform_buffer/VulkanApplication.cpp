...
VulkanApplication::VulkanApplication(const char *app_name, const int width, const int height)
    : initialized(false)
{
    if (!glfwInit())
    {
...
bool VulkanApplication::createSwapChain()
{
    ...
    const VkSwapchainCreateInfoKHR createInfo{
            ...
            getSurface(),
            caps.minImageCount,
            surfaceImageFormat.format, imageFormat[0].colorSpace,
            caps.maxImageExtent,
            1,
...
bool VulkanApplication::createSwapChainImageView()
{
    ...
    for (size_t i = 0; i < imageSize; i++)
    {
        ...
        if (const auto res = vkCreateImageView(getLogicDevice(), &imageViewCreateInfo, nullptr, &swapChainImageViews[i]);
            res != VK_SUCCESS)
        {
...
bool VulkanApplication::setUpGraphicsPipeline(const std::string& vertex_shader_path, const std::string& fragment_shader_path,
                                              ...
                                              VkPipelineInputAssemblyStateCreateInfo &vertex_input_assembly, VkDescriptorSetLayout desc_set)
{
    ...
    auto res = vkCreateGraphicsPipelines(getLogicDevice(),VK_NULL_HANDLE,1, &pipeline_create_info, nullptr, &graphicsPipeline);
    if (res != VK_SUCCESS)
    {
...
uint32_t VulkanApplication::reCreateSwapChain(int &w, int &h, VkSemaphore &wait_image)
{
    ...
    {
        /// 重新请求下一张图
        waitRes = vkAcquireNextImageKHR(getLogicDevice(), swapChain, UINT64_MAX, wait_image,VK_NULL_HANDLE, &imageIndex);
        if (waitRes!=VK_SUCCESS)
        {
