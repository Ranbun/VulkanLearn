...
VulkanApplication::VulkanApplication(const char *app_name, const int width, const int height) : initialized(false)
{
    if (!glfwInit())
    {
    ...
    initialized = createInstance(app_name);

    if (initialized)
    {
        initialized = obtainPhysicalDevice();
    }

    if (initialized)
    {
        initialized = createLogicDevice();
    }

    if (initialized)
    {
        initialized = createWindowSurface(app_name, width, height);
    }

    if (initialized)
    {
        initialized = createSwapChain();
    }

    if (initialized)
    {
        initialized = createSwapChainImageView();
    }

    if (initialized)
    {
        initialized = createCommandBuffer();
    }

    if (initialized)
    {
        initialized = createRenderPass();
    }
    ...
    if (initialized)
    {
        initialized = createFrameBuffer(width, height);
    }
}
...
VulkanApplication::~VulkanApplication()
{
    ...
    if (pipelineLayout)
    {
        vkDestroyPipelineLayout(getLogicDevice(), pipelineLayout, nullptr);
    }
    ...
    {
        /// 之前必须保证所有命令都被提交
        vkDestroyRenderPass(getLogicDevice(), renderPass, nullptr);
    }

    if (commandPool)
    {
        vkDestroyCommandPool(getLogicDevice(), commandPool, nullptr);
    }

    cleanUpSwapChain();

    for (const auto &fence: fences | std::views::values)
    {
        vkDestroyFence(getLogicDevice(), fence, nullptr);
    }

    for (const auto &semaphore: semaphores | std::views::values)
    {
...
bool VulkanApplication::createInstance(const char *app_name)
{
    /// create application
    VkApplicationInfo appinfo{
            VK_STRUCTURE_TYPE_APPLICATION_INFO, nullptr,           app_name, VK_MAKE_VERSION(1, 0, 0), "VulkanClass",
    ...
    const std::vector<const char *> layers_name{"VK_LAYER_KHRONOS_validation"};

    VkInstanceCreateInfo vkInstanceCreateInfo{VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO, nullptr, 0, &appinfo, 0, nullptr,
                                              // 1, layersName,
                                              static_cast<uint32_t>(extensions.size()), extensions.data()};

    vkInstanceCreateInfo.enabledLayerCount = 1;
    vkInstanceCreateInfo.ppEnabledLayerNames = layers_name.data();
...
bool VulkanApplication::createLogicDevice()
{
    ...
    const auto result = vkCreateDevice(getPhysicalDevice(), &logicDeviceCreateInfo, nullptr, &logicDevice);
    if (result != VK_SUCCESS)
    {
...
bool VulkanApplication::createWindowSurface(const char *name, int width, int height)
{
    ...
    VkResult result = glfwCreateWindowSurface(GetInstance(), window, nullptr, &surface);
    if (result != VK_SUCCESS)
    {
...
bool VulkanApplication::createSwapChain()
{
    ...
    result = vkGetPhysicalDeviceSurfacePresentModesKHR(getPhysicalDevice(), getSurface(), &presentModeCount,
                                                       imagePresentMode.data());
    if (result != VK_SUCCESS)
    {
        return false;
    }
    ...
    presentMode = imagePresentMode[0];
    surfaceImageFormat = imageFormat[0];
    ...
    const VkSwapchainCreateInfoKHR createInfo{VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR,
                                              ...
                                              getSurface(),
                                              caps.minImageCount,
                                              surfaceImageFormat.format,
                                              imageFormat[0].colorSpace,
                                              caps.maxImageExtent,
                                              1,
    ...
    result = vkCreateSwapchainKHR(getLogicDevice(), &createInfo, nullptr, &swapChain);
    if (result != VK_SUCCESS)
    {
    ...
    /// 记录当前窗口的大小
    extent = caps.maxImageExtent;

    uint32_t imageCount = 0;
    vkGetSwapchainImagesKHR(getLogicDevice(), swapChain, &imageCount, nullptr);

    swapChainImages.resize(imageCount);
    vkGetSwapchainImagesKHR(getLogicDevice(), swapChain, &imageCount, swapChainImages.data());
...
bool VulkanApplication::createSwapChainImageView()
{
    ...
    swapChainImageViews.resize(imageSize);

    for (size_t i = 0; i < imageSize; i++)
    {
        imageViewCreateInfo.image = getSwapChainImage()[i];
        if (const auto res =
                    vkCreateImageView(getLogicDevice(), &imageViewCreateInfo, nullptr, &swapChainImageViews[i]);
            res != VK_SUCCESS)
        {
...
bool VulkanApplication::createCommandBuffer()
{
    ...
    /// 创建指令池
    auto res = vkCreateCommandPool(getLogicDevice(), &poolInfo, nullptr, &commandPool);
    if (res != VK_SUCCESS)
    {
    ...
    // 从指令池中获取一个指令缓冲
    res = vkAllocateCommandBuffers(getLogicDevice(), &allocInfo, &commandBuffer);
    if (res != VK_SUCCESS)
    {
...
bool VulkanApplication::createRenderPass()
{
    ...
    /// 創建子流程
    VkSubpassDescription subpass{0,       VK_PIPELINE_BIND_POINT_GRAPHICS, /// 指定当前的这个子流程是图形管线的子流程
                                 0,       nullptr,
    ...
    const auto res = vkCreateRenderPass(getLogicDevice(), &renderPassInfo, nullptr, &renderPass);
    if (res != VK_SUCCESS)
    {
...
{
    frameBuffers.resize(getSwapChainImageView().size());
    for (auto i = 0; i < frameBuffers.size(); i++)
    {
        ...
        const auto res = vkCreateFramebuffer(getLogicDevice(), &info, nullptr, &frameBuffers[i]);
        if (res != VK_SUCCESS)
        {
            return false;
        }
...
void VulkanApplication::cleanUpSwapChain() const
{
    ...
    for (const auto _swapChainImageView: swapChainImageViews)
    {
    ...
    {
        vkDestroySwapchainKHR(getLogicDevice(), swapChain, nullptr);
    }
}

bool VulkanApplication::setUpGraphicsPipeline(const std::string &vertex_shader_path,
                                              const std::string &fragment_shader_path,
                                              VkPipelineVertexInputStateCreateInfo &vertex_input,
                                              VkPipelineInputAssemblyStateCreateInfo &vertex_input_assembly,
                                              VkDescriptorSetLayout desc_set)
{
    auto vertexShaderModel = createShaderModule(vertex_shader_path);
    auto fragShaderModel = createShaderModule(fragment_shader_path);

    /// 可编成阶段配置
    /// 创建着色器阶段
    VkPipelineShaderStageCreateInfo shaderStages[] = {{
                                                              VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
                                                              nullptr, 0, VK_SHADER_STAGE_VERTEX_BIT, vertexShaderModel,
                                                              "main", nullptr // 通过指定常量优化效率
                                                      },
                                                      {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, nullptr, 0,
                                                       VK_SHADER_STAGE_FRAGMENT_BIT, fragShaderModel, "main", nullptr}};
    ...
    viewportStates.emplace_back(viewport);

    std::vector<VkRect2D> scissorStates;
    scissorStates.emplace_back(scissor);
    ...
    colorBlendAttachmentStates.emplace_back(colorBlendAttachment);
    ...
    if (VK_SUCCESS != vkCreatePipelineLayout(getLogicDevice(), &pipelineLayoutCreateInfo, nullptr, &pipelineLayout))
    {
    ...
    pipeline_create_info.pInputAssemblyState = &vertex_input_assembly;
    ...
    auto res = vkCreateGraphicsPipelines(getLogicDevice(), VK_NULL_HANDLE, 1, &pipeline_create_info, nullptr,
                                         &graphicsPipeline);
    if (res != VK_SUCCESS)
    {
...
void VulkanApplication::keyPressCallBack(GLFWwindow *window, int key, int scancode, int action, int mods) const
{
    (void) this;
...
void VulkanApplication::resizeCallBack(GLFWwindow *window, const int w, const int h) const
{
    (void) this;
...
void VulkanApplication::submitAndPresent(VkSemaphore waitImage, VkSemaphore waitSubmission, VkFence fenceSubmission,
                                         uint32_t imagIndex)
{
    ...
    VkSubmitInfo submitInfo{
            ...
            &commandBuffer,
            1,
            &waitSubmission // 提交的指令缓存  执行完成之后通知这个信号量  告诉等待这个信号量的地方开始执行
    };

    vkQueueSubmit(queue, 1, &submitInfo, fenceSubmission);

    VkPresentInfoKHR presentInfo{
            VK_STRUCTURE_TYPE_PRESENT_INFO_KHR, nullptr, 1, &waitSubmission, 1, swapChains, &imagIndex, nullptr};
...
uint32_t VulkanApplication::reCreateSwapChain(int &w, int &h, VkSemaphore &wait_image)
{
    cleanUpSwapChain();
    // 等待设备空闲得时候重新创建交换链
    auto waitRes = vkDeviceWaitIdle(logicDevice);
    ...
    if (created)
        created = createSwapChainImageView();
    if (created)
        created = createFrameBuffer(w, h);
    ...
    if (created)
    {
        /// 重新请求下一张图
        waitRes =
                vkAcquireNextImageKHR(getLogicDevice(), swapChain, UINT64_MAX, wait_image, VK_NULL_HANDLE, &imageIndex);
        if (waitRes != VK_SUCCESS)
        {
